...
function updateThreshold(){
  ...
  document.querySelector("#scoreRows").innerHTML=cases.map(item=>{const predicted=item.score>=threshold;return `<div class="score-item ${predicted?"predicted":""} ${item.positive?"actual-positive":"actual-negative"}" title="${item.id}: score ${item.score.toFixed(2)}, actually ${item.positive?"late":"on time"}, predicted ${predicted?"escalate":"do not escalate"}"><strong>${item.score.toFixed(2)}</strong><small>${predicted?"Escalate":"No action"}</small></div>`;}).join("");
